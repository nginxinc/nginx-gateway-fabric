...
| NGINX errors                 | Reload failures on NGINX               | Check logs for _nginx_ container                   | nginx container does not have the proper permissions.                 |
...
Note that the port number i.e `8080` matches the port number you have port-forwarded for your NGINX Gateway Fabric Pod. For further details, see the [dashboard guide]({{< relref "dashboard.md" >}}).
